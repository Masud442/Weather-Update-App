...
import 'package:weather_app_pb_fl_01/models/current_model.dart';
import 'package:weather_app_pb_fl_01/models/forecast_model.dart';
...
class WeatherProvider with ChangeNotifier {
  ...
  Future<void> _getCurrentData() async {
    final uri = _city == null ? Uri.parse('http://api.openweathermap.org/data/2.5/weather?lat=$_lat&lon=$_lon&units=$_unit&appid=$weather_api_key') :
    Uri.parse('http://api.openweathermap.org/data/2.5/weather?q=$_city&units=$_unit&appid=$weather_api_key');
    try{
  ...
  Future<void> _getForecastData() async {
    final uri = _city == null ? Uri.parse('http://api.openweathermap.org/data/2.5/forecast?lat=$_lat&lon=$_lon&units=$_unit&appid=$weather_api_key') :
    Uri.parse('http://api.openweathermap.org/data/2.5/forecast?q=$_city&units=$_unit&appid=$weather_api_key');
    try{
      final response = await http.get(uri);
      final map = json.decode(response.body);
      print('PRINTING MAP FOR NULL: ${map['list']}');
